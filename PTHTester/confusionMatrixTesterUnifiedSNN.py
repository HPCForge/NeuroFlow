...
model = VGG_NN().to('cuda')
model.load_state_dict(torch.load("./unifiedEventModelSNNBasic15E55.pth",torch.device('cuda')))
model = model.to('cuda')
model.eval()
...
plt.title('Event SNN', fontsize="40")
ax = sn.heatmap(df_cm, annot=True, annot_kws={"size": 32}, cbar=False, fmt=".2g")
ax.set_yticklabels(ax.get_yticklabels(), fontsize=18, rotation=0)
ax.set_xticklabels(ax.get_xticklabels(), fontsize=18)
plt.savefig('unifiedSNNReordered.png')
print(df_cm)
